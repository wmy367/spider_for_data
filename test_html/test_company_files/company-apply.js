...
(function () {
    ...
    zlapply.searchjob.all_ele = ["t", "st", "i", "d", "l", "k", "s", "m", "p", "s_f", "s_t", "kt", "w", "e", "ct", "cs", "et", "ga", "gc", "gr", "re", "bj", "sj","isfilter","fl","fjt"];
    zlapply.searchjob.v = {
        h_n: "vacancyid",
        q_n: "vacancyid"
    };
    ...
    zlapply.searchjob.checkLoginFormCollect = function (nextType) {
        ...
        var goto = true;
//        var data = apply.paraName.type + "=2";//zh
        if (l.trim() == "") {
            l_html.innerHTML = "请输入用户名";
            goto && (goto = false)
        } else {
            l_html.innerHTML = "";
//            data += "&" + apply.paraName.username + "=" + l.urlEncode()
        }
        if (p.trim() == "") {
            ...
            p_html.innerHTML = "";
//            data += "&" + apply.paraName.password + "=" + p.urlEncode()
        }
        ...
        if($("validateLi").style.display == "block"){
            if (v.trim() == "") {
                ...
                v_html.innerHTML = "";
//                data += "&" + apply.paraName.validator + "=" + v.urlEncode()
            }
        }else{
//            data += "&" + apply.paraName.validator + "= 1";
        }

//        if (i.checked) data += "&" + apply.paraName.isautologin + "=1";
//        if (goto) {
//
//            apply.showMask();
//            jsonp({
...
//                                break
//                        }
//                        if(apply.getStatusIndex(data["needpic"]) ==="y"){
//                            if($("validateLi").style.display  =="none"){
//                                $("validateLi").style.display ="block";
//                                $("validateErrCon").innerHTML = "";
//                                $("passwordErrCon").innerHTML ="";
...
//        }
    ...
    zlapply.searchjob.gotoOkPage = function (data) {
        ...
        if((rapp7 == 0 && rapp30 == 0) || sstj == "1"){
            try{
                //由于jobs页面可能没刷新过来，新加载的js文件还没有，所以加异常捕获
                ZPIDC.applyjob.feedBackWeiXin();
            }catch(e){}
            jQuery('.popupApply').hide().css('opacity','1');
            jQuery('.divMask').remove();
            if (box.length===0) {
                var htm = '<div id="applyOkay" style="top:266px;left:50%;margin-left:-136px;text-align:center;border:3px solid #b0b0b0;width:272px;height:53px;background:url(http://img03.zhaopin.cn/2012/img/icon/applyOkay.png) no-repeat 83px 11px #fff;color:#555;z-index:1110;position:fixed;"></div>';
                if (window.XMLHttpRequest){
                    jQuery(htm).appendTo(jQuery('body')).delay(750).fadeOut();
                } else {
                    _top = jQuery(window).scrollTop() + 266;
                    jQuery(htm).css('position','absolute').css('top',_top).appendTo(jQuery('body')).delay(750).fadeOut();
                }
            } else {
                if (window.XMLHttpRequest){
                    jQuery(box).show().delay(750).fadeOut();
                } else {
                    _top = jQuery(window).scrollTop() + 266;
                    jQuery(box).css('top',_top).show().delay(750).fadeOut();
                }
            }
            sstj = "" ;
        }else{
        ...
        jQuery.getJSON("http://my.zhaopin.com/jobseeker/getJobListcom.asp?format=json&c=?",datastr,
            function(data){
                ...
                if(cont>1){
                    ...
                    // if(rapp7>0){jQuery(".applyyn1").show()}else{jQuery(".applyyn3").show();}
    ...
    apply.genHTML = function (type, data, argu) {
        ...
        switch (type) {
            ...
            case "14":
                ...
                break;
            case "15":
                html += "<div id=\"loginBlock\"><form name=\"loginForm\"><ul><li><label class=\"leftLabel\">用户名：<input type=\"text\" id=\"simplaceholder1\" class=\"textInput\" name=\"loginname\"  maxlength=\"100\" value=\"输入手机号/邮箱\" style=\"color:#999;\"/></label><label><input type=\"checkbox\" name=\"isautologin\" value=\"1\" />自动登录</label></li><li class=\"tips\" id=\"loginnameErrCon\"></li><li><label class=\"leftLabel\">密<img src=\"" + piximg + "\" width=\"12\" height=\"1\" />码：<input type=\"password\" class=\"textInput\" name=\"password\" value=\"\" maxlength=\"25\" /></label><a href=\"http://passport.zhaopin.com/findpassword/email/step1\" target=\"_blank\">忘记密码</a></li><li class=\"tips\" id=\"passwordErrCon\"></li><li id=\"validateLi\" style=\"display:none\"><label class=\"leftLabel\">验证码：<input type=\"text\" class=\"textInput\" name=\"validate\" id=\"validate\" value=\"\" maxlength=\"5\" /></label><a title=\"刷新验证码\" href=\"#\" onClick=\"ZPIDC.applyjob.freshValidate();return false;\"><img align=\"absmiddle\" id=\"vimg\" alt=\"看不清？点击更换\"></a><a title=\"刷新验证码\" href=\"#\" onClick=\"ZPIDC.applyjob.freshValidate();return false;\">看不清，换一张</a></li><li class=\"tips\" id=\"validateErrCon\"></li></ul><div class=\"greyDottedLineH\"></div><div id=\"submitBlock\" style=\"margin-left: 108px;width:264px;\"><div class=\"leftFBlock\"><a href=\"#\" class=\"popapplyButton\" onclick=\"zlapply.searchjob.checkLoginFormCollect();return false;\">登&nbsp;&nbsp;录</a></div><div class=\"rightFBlock\"><a href=\"http://passport.zhaopin.com/account/register\" class=\"txtLink\" target=\"_blank\" style=\"display: block; width: 164px; height: 29px; font-size: 14px; font-weight: bold; text-decoration: none; background: url('http://img02.zhaopin.cn/2012/img/ui/register_jobs.png') no-repeat scroll 0px 0px transparent; color: #3359AA;\">快速注册，申请职位</a></div><div class=\"clear\"></div></div></form></div>";
                break;
            case "18" ://简历推荐
    ...
    apply.buildDivFrame = function () {
        ...
        try{
            if(zlzp && zlzp.searchjob.ajaxApply){
    ...
    zlapply.searchjob.collect = function(type,definPro){
        ...
        var sTop = jQuery(window).height(), sLeft = jQuery(window).scrollLeft();
        var wHeight = jQuery(window).height(), wWidth =jQuery(window).width();
        var y = wHeight / 2 - souCollect.height() / 2;
        var x = wWidth / 2 - souCollect.width() / 2;
        switch (type) {
            case 0:
