...
con <- dbConnect(sqlite,"/home/young/work/ruby/spider_for_data/db/database.sqlite3")
new_spider_date <- dbGetQuery(con,'select * from spider_dates')
new_spider_date <- data.frame(id=new_spider_date$id,search_key=new_spider_date$search_key,search_location=new_spider_date$search_location,stringsAsFactors=FALSE)
...
# query_str <- sprintf("select * from zhilian_jobs where (zhilian_jobs.search_key =\"UI\" and zhilian_jobs.spider_date_id =\"1\" )")
jobs <- dbGetQuery(con,query_str)

key_str <- new_spider_date[new_spider_date$id==new_spider_date_id,]$search_key
location_str <- new_spider_date[new_spider_date$id==new_spider_date_id,]$search_location
